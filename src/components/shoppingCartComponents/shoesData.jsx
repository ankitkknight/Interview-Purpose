...
const ShoesData = ({ data, addToSaveLater, setSelectedId }) => {
  {
    console.log(data);
  }
  return (
    <Box sx={{ display: { xs: "none", lg: "block" } }}>
      ...
      <Box sx={styles.rootContainer}>
        {data.map((item) => {
          return (
            <Box sx={styles.rootChildContainer} key={item.id}>
              {console.log({ item })}
              ...
              <Box sx={{ marginLeft: "1%" }}>
                ...
                <Typography sx={styles.colorBlue}>
                  Color : {item.color}
                </Typography>
              ...
              <Typography sx={styles.priceStyles}>Rs. {item.price}</Typography>
              <Box sx={styles.counterBox}>
                <CounterBox />
              </Box>
              <Typography sx={styles.finalPrice}>Rs. {item.price}</Typography>
            </Box>
          );
        })}
      </Box>
